...
async function handleLogin() {
  ...
  // Récupérer le mot de passe depuis Supabase settings (ou fallback CONFIG)
  let storedPassword = CONFIG.CRM_PASSWORD;
  try {
    const dbPw = await Settings.get('crm_password');
  ...
  if (password === storedPassword) {
    ...
    try {
      const userName = await Settings.get('user_name');
      if (userName) sessionStorage.setItem('crm_user_name', userName);
    } catch {}

    // Charger la clé Google Maps
    await loadGoogleMapsKeyFromSettings();
